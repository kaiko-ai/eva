"""Tests for JSON free-form prompt template."""
...
def test_render_basic_trims_input(template: JsonFreeFormQuestionPromptTemplate) -> None:
    ...
    assert "IMPORTANT: Respond with a valid JSON object" in result
...
def test_render_with_examples() -> None:
    ...
    assert "Example JSON Answer:" not in result
...
def test_render_enable_cot_with_examples_shows_instruction() -> None:
    ...
    # CoT instruction should appear even when examples are provided (unlike XML template)
