...
def test_double_bos_removal(mock_vllm_imports, monkeypatch):
    ...
    class MockTokenizerDoubleBOS(MockTokenizer):
        def apply_chat_template(self, _messages, tokenize=True, add_generation_prompt=True):  # noqa: ARG002
            # Return flat list with double BOS at the beginning
            return [1, 1, 2, 3, 4]
    ...
    # Capture log to verify warning is issued
    import logging
    with monkeypatch.context() as m:
        ...
        # Verify we get some valid result
...
def test_no_chat_template_error(mock_vllm_imports, monkeypatch):
    ...
    class MockTokenizerNoTemplate:
        def __init__(self):
            pass  # No chat_template attribute
