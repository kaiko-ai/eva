...
class LUNA25(VisionDataset[Volume, torch.Tensor]):
    ...
    def _get_split_indices(self) -> list[int]:
        train_indices, val_indices, test_indices = stratified_split(
            samples=self._candidates.index.tolist(),
            targets=self._candidates["label"].values,
            groups=self._candidates["PatientID"].values,
            train_ratio=1.0 - self._val_size - self._test_size,
            val_ratio=self._val_size,
            test_ratio=self._test_size,
            seed=42,
        )
        ...
        subset_indices = splits.get(self._split)
