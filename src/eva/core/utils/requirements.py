...
def check_dependencies(requirements: Dict[str, str]) -> None:
    ...
    Args:
        requirements: A dictionary where keys are package names and values are minimum required versions.
    ...
    for package, min_version in requirements.items():
        ...
        if actual and not (version.parse(actual) >= version.parse(min_version)):
            raise ImportError(
                f"Package '{package}' version {actual} does not meet the minimum required version {min_version}."
            )
